  ...
  <div class="tag-page">
    ...
    <template v-else>
      <header class="page-header">
        <h1 class="page-title">
          <Icon name="mdi:tag-outline" class="title-icon" />
          标签：{{ tagName }}
        </h1>
...
// 获取该标签下的文章
const { data: articles, pending, error } = await useFetch(`/api/articles?tag=${slug}`)

// 从第一篇文章中获取标签名称（如果有文章的话）
const tagName = computed(() => {
  if (articles.value && articles.value.length > 0 && articles.value[0].tags && articles.value[0].tags.length > 0) {
    const tag = articles.value[0].tags.find((t: any) => t.slug === slug)
